...
import GSlogo from "../../../assets/images/logo.png";
...
function BrandsProduct({handleAddComp}) {
    ...
    <div className='bestsaler'>
      ...
      <Row className=''>
        <Col>
          {b!=null && <h1 className='bestsaler-brandname'>{b.brand.name}</h1>} 
        </Col>
      </Row>
      </Row> 
    <Row>
        <div className='bestsaler-container'>
            {item.map((item,index) => (
                    <div className='bestsaler-container-item' key={index}>
                        <ProductDetailsItems id={item.id} name={item.name} price={item.price} status={item.status} brand={item.brand.name} images={item.images[0].path} handleAddComp={handleAddComp}/>
                    </div>
            ))}
        </div>
        </Row>
